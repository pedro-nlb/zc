...
\begin{thm}[Hairy ball theorem]
	...
	\begin{enumerate}[label=\textbullet]
	    ...
	    \item For $\vec{v}\in \R^{2}\setminus \{ 0 \}$ we denote by $R_{\vec{v}}\colon \R^{2}\to \R^{2}$ the reflection with respect to the direction defined by $\vec{v}$.
	\end{enumerate}

	We prove the theorem by contradiction, so suppose that we are given a non-vanishing continuous vector field $X$ on $\bbS^{2}$.
	...
	Let $\vec{v}_{p}$ denote the corresponding tangent vector $X(h(p))$.
	We consider the line $L'_{p}$ which is parallel to $L_{p}$ and passes through the point $h(p)+\vec{v}_{p}\in \R^{3}$.
	...
	The functions $f,f^{*}\colon \bbS^{1}\to \bbS^{1}$ are also related to each other.
	Namely, ...
