...
Suppose that $h\colon B\to C$ is a ring isomorphism.
...
\begin{lm}\label{lm:frd}
    The ring $A$ is a formally real domain, i.e.~$0$ is not a sum of non-zero squares.
\end{lm}

It follows by looking at the leading coefficients in a sum of squares that polynomial rings over $A$ are also formally real domains.
Again by looking at the leading coefficients in a sum of squares we see that all the solutions of $t_{1}^{2}+t_{2}^{2}+t_{3}^{2}=1$ in $D[t]^{\op 3}$ are in $D^{\op 3}$ for any formally real domain $D$.
This implies that all the solutions of such an equation in $A[\alpha,\beta,t]^{\op 3}=B[t]^{\op 3}$ are in $A^{\op 3}$.
Therefore all the solutions of such an equation in $B^{\op 3}$ or in $C^{\op 3}$ are also in $A^{\op 3}$.
As a consequence, $h(A)\subseteq A$.
Indeed, it suffices to show that $h(x+\p),h(y+\p),h(z+\p)\in A$, and this follows from the previous remark and the observation that
...
Hence $C$ can be generated as an $A$-algebra by two elements $c,c'\in C$, i.e.~$C\cong A[c,c']$ over $A$.
