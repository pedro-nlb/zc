...
\begin{abstract}
    Following \cite{hoc72} we provide an example of rings (commutative and with $1$ throughout the talk) $R,R'$ such that $R[t]\cong R'[t]$ but $R\not\cong R'$, where $t$ is an indeterminate.
...
\begin{center}
    Many thanks to Oliver Braunling for his help while preparing this talk!
...
\begin{thm}[Hairy ball theorem]\label{thm:hairyball}
	...
	\end{align*}
	But this is a contradiction, because $c^{*}\colon \bbS^{1}\to \bbS^{1}$ is the morphism going twice around $\bbS^{1}$ at constant speed, which has degree $2$.
	\begin{figure}[htp]
	    \centering
	    \includegraphics[scale=.24]{circle.png}
	\end{figure}
...
{\color{gray}
If we take the Whitney embedding theorem \cite[Theorem II.10.7]{bre93}, the tubular neighbourhood theorem \cite[Theorem II.11.4]{bre93} and the Lefschetz--Hopf fixed point theorem \cite[Theorem IV.23.4]{bre93} for granted, we can prove a more general statement:
...
Indeed, both $A$-algebras have the same universal property, because the functor
...
In this subsection we make sure that $h$ has to be an $\R$-algebra isomorphism.
...
We have $C\subseteq C[t]\cong B[t]$ over $A$, so we can see both $B$ and $C$ as $A$-subalgebras inside the same $A$-algebra $B[t]=A[\alpha,\beta,t]$.
...
Let now $e\in C$ be a homogeneous element in degree $1$.
